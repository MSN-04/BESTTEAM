...
public class ReviewViewAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int review_num = Integer.parseInt(request.getParameter("review_num")); // String -> int 형변환 필요
		String page = request.getParameter("page");
		
		ReviewViewService reviewViewService = new ReviewViewService();
		ReviewBean article = reviewViewService.getArticle(review_num);
		...
		forward.setPath("./shop/review_view.jsp"); // 이동할 jsp 페이지 지정
