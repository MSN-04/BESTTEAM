...
@WebServlet("*.no") // => URL 요청에 마지막이 *.no 로 끝나는 요청을 매핑하는 서블릿 지정(web.xml 설정 대신 사용)
...
//			System.out.println("noticeList.no");
			...
			// 글 목록 보기 작업 요청은 비즈니스 로직 처리를 위한 noticeListAction() 인스턴스 생성 필요
...
//			System.out.println("noticeDetail.no");
			
			// 글 상세내용 보기 작업 요청은 비즈니스 로직 처리를 위한 noticeDetailAction() 인스턴스 생성 필요
			...
			// 수정글 관련 정보 가져오기 - noticeModifyFormAction 클래스로 이동
